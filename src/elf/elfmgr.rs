...
static RE_VAR: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(anonymous namespace)|@GLIBC|std::|_IO_stdin_used|^\._|^__gnu_|^__cxxabiv|^guard variable|\)::__func__$|\.\d+$").expect("Lazy<Regex> init failed")
});
